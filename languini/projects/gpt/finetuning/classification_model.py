...
class HFClassificationModel(PreTrainedModel):
    ...
    def __init__(self, config: HFClassificationModelConfig):
        ...
        assert c.frac_duplicated in [0, 1], "not implemented"
        assert not c.dedup_type, "not implemented"
        assert not (c.frac_duplicated == 0 and c.L_train == "L2"), "not implemented"
    ...
    def set_prediction_language(self, L_predict):
        ...
        if self.config.frac_duplicated == 0:
            raise ValueError("Cannot predict in other language if models wasn't trained under duplication")
    ...
    def to_language(self, input_ids):
        ...
        if L == "L1":
            ...
            return torch.where(self.is_duplicated[input_ids], input_ids + self.config.vocab_size, input_ids)
        else:
    ...
    def forward(self, input_ids, labels=None, return_dict=None):
        ...
        # classification head on last hidden state
        logits = self.classifier(self.dropout(hidden_states[:, -1, :]))
